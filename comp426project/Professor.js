...
const professor_data = require('data-store')({path: process.cwd()+'/data/professor.json'});
...
Professor.create = (firstName, lastName, email, user, password, classes, students ) => {
    let id = Professor.next_id; 
    Professor.next_id +=1; 
    let p= new Professor(id, firstName, lastName, email,user,  password, classes, students);
